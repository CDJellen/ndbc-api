...
class RequestHandler(metaclass=Singleton):
    ...
    def add_station(self, station_id: Union[str, int]) -> None:
        ...
        self.stations.append(RequestHandler.Station(station_id=station_id, cache_limit=self._cache_limit))

    def build_request(self, station_id: Union[str, int], mode: str, start_time: datetime, end_time: datetime) -> Union[str, List[str]]:
        if start_time > end_time:
            raise ValueError
        if mode == 'stdmet':
            return self.build_request_stdmet(station_id, start_time, end_time)
        elif mode == 'cwind':
            return self.build_request_cwind(station_id, start_time, end_time)
        else:
            raise NotImplementedError

    def build_request_stdmet(self, station_id: Union[str, int], start_time: datetime, end_time: datetime) -> str:
        is_historical = (datetime.now()-start_time) >= timedelta(days=44)
        requested_station = str(station_id)
        if is_historical:
            return self.build_request_stdmet_historical(requested_station, start_time, end_time)
        return self.build_request_stdmet_realtime(requested_station)

    def build_request_stdmet_historical(self, requested_station: str, start_time: datetime, end_time: datetime) -> str:
        ...
        if has_realtime:
            reqs.append(self.build_request_stdmet_realtime(requested_station=requested_station))
    ...
    def build_request_cwind(self, station_id: Union[str, int], start_time: datetime, end_time: datetime) -> str:
        is_historical = (datetime.now()-start_time) >= timedelta(days=44)
        requested_station = str(station_id)
        if is_historical:
            return self.build_request_cwind_historical(requested_station, start_time, end_time)
        return self.build_request_cwind_realtime(requested_station)

    def build_request_cwind_historical(self, requested_station: str, start_time: datetime, end_time: datetime) -> str:
        def req_hist_helper_year(req_year: int) -> str:
    ...
    def handle_requests(self, station_id: Union[str, int], reqs: List[str]) -> List[str]:
        responses = []
        for req in reqs:
