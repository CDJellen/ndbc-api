...
class NdbcApi(metaclass=Singleton):
    ...
    def get_data(
        ...
        if not data_api_call:
            raise NotImplementedError('Please supply a supported mode from `get_modes()`.')
        try:
            return data_api_call(self._handler, station_id, cols, start_time, end_time, use_timestamp, as_df)
